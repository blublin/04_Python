...
'''
def iterateDictionary(someList: list):
    for aDict in someList:
        dictList = []
        for k,v in aDict.items():
            dictList.append(f"{k} - {v}")
        print(", ".join(dictList))
students = [
    {'first_name':  'Michael', 'last_name' : 'Jordan'},
    {'first_name' : 'John', 'last_name' : 'Rosales'},
...
def iterateDictionary2(keyName: str, someList: list):
    for aDict in someList:
        print(aDict.get(keyName, f"{keyName} not found in {aDict}."))
...
'''
def printInfo(someDict: dict):
    for k,v in someDict.items():
        print(len(someDict[k]), k)
